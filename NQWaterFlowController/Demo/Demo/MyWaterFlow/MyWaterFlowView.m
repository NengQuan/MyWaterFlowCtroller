...
{
    // 总列数
    NSUInteger numerOfColumns = [self numberOfColumns];
    ...
    return (self.bounds.size.width - leftM - rightM - (numerOfColumns - 1) * columnM) / numerOfColumns;
}
...
{
    return (CGRectGetMaxY(frame) > self.contentOffset.y) && (CGRectGetMidY(frame) < self.contentOffset.y + self.bounds.size.height);
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self.displayingCells enumerateKeysAndObjectsUsingBlock:^(id  key, MyWaterFlowVIiewCell *cell, BOOL * _Nonnull stop) {
